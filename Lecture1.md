...
1. In the VS Code file explorer on the left, create a new folder for your course work (e.g., right-click in the blank space and choose New Folder... named cpp\_course).  
2. Right-click your new cpp\_course folder and select New File....  
...
5. **Save the file.** (It might auto-save). You've now *written* a C++ program. But you can't *run* it... yet.
...
2. **Compile Your Code:** In the terminal, type the following command and press Enter:  
   g++ \-o hello hello.cpp
...
3. **Look at Your Files:**  
   * In the VS Code file explorer on the left, look inside your cpp\_course folder (or type ls in the terminal). You will now see **two** files:  
     * hello.cpp (your source code)  
     * hello \- This is your new **executable program**\!  
4. **Run Your Program:** In the terminal, type the following:  
   ./hello
...
5. **Observe the Error:** The compiler will stop and print an error message. It will say something like error: expected ';' before 'return'. It tells you *exactly* what's wrong and on what line.  
6. This is a **compile-time error**. Your program *was not created*. The old hello executable (if it existed) is still there, unchanged.
...
3. Save the file and try to compile: g++ \-o hello hello.cpp  
4. **Observe the Error:** You'll get a much more complex error, like no match for 'operator+' (operand types are 'const char\*' and 'int').  
...
1. Create a new file bad.py in your cpp\_course folder.  
